...
    // --- START: UPDATED SQL QUERY FOR CORRECT FILTERING AND IMAGE HANDLING ---
    $sql = "
    SELECT 
        tr.transaction_id,
        tr.status,
...
        GROUP_CONCAT(img.image_path) AS image_paths -- Use GROUP_CONCAT to collect all images in one row
...
    WHERE 
        -- Winning Bids (User is the buyer, item is a bid, and transaction is successful)
        (tr.buyer_id = ? AND tr.item_type = 'bid' AND tr.status = 'successful') 
        OR 
        -- Successful Swaps (User is the buyer or seller, item is a swap, and transaction is successful)
        ((tr.buyer_id = ? OR tr.seller_id = ?) AND tr.item_type = 'swap' AND tr.status = 'successful')
        
    GROUP BY tr.transaction_id -- Group results to fix image duplication
    ORDER BY tr.transaction_id DESC
    ";

    $stmt = $conn->prepare($sql);
    // Bind three parameters now, as there are three '?' in the WHERE clause
...
    $bids = [];
    $swaps = [];
    // $images array initialization is no longer needed
    

    while ($row = $result->fetch_assoc()) {
        // --- START: UPDATED PHP IMAGE PROCESSING ---
        // Process images using GROUP_CONCAT (from the 'image_paths' column)
...
        // --- END: UPDATED PHP IMAGE PROCESSING ---
