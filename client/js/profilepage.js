...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function displayUserInfo() {
        ...
        const idEl = document.getElementById("user-id-placeHolder");
        const emailEl = document.getElementById("user-email-placeHolder");

        if (nameEl) nameEl.textContent = userInfo.username;
        if (idEl) idEl.textContent = userInfo.user_id;
    ...
    function generateProfilePicture(username, size = 300) {
        ...
        ctx.fillText(letter, size / 2, size / 2 + (fontSize * 0.08));
    ...
    function fetchListings() {
        ...
        .then(data => {
            ...
            listings = data.listings.map(item => ({
                ...
                endDate: item.end_date, // Added for countdown synchronization
                status: item.status,
            }));
            ...
            const counter = document.querySelector("#profile-listings-btn p");
            if (counter) counter.textContent = `${listings.length} ${listings.length === 1 ? "Item" : "Items"}`;
        })
    ...
    function createListingRow(listing) {
        ...
        // Timer Logic: If it's an active auction, show countdown. Otherwise show listed date.
    ...
    // New Function to start live timers on the profile page
    ...
    function renderListings(mode) {
        ...
        if (filtered.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">No ${mode} listings available</td></tr>`;
        } else {
            filtered.forEach(listing => {
                tbody.appendChild(createListingRow(listing));
            });
            // Start the countdown logic for newly rendered rows
            startProfileCountdowns();
        }
    }
    ...
    function fetchWinningBids() {
        fetch('../server/item/get_transactions.php', { method: 'GET', credentials: 'include' })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    ...
                    const counter = document.querySelector("#profile-bids-btn p");
                    if (counter) counter.textContent = `${winningBids.length} Items`;
                }
            });
    }
    ...
    function createWinningBidRow(bid) {
        const row = document.createElement("tr");
        row.innerHTML = `
            <td class="item">${bid.item}</td><td>${bid.category}</td>
            <td>${bid.winningBid}</td><td>${bid.vendor}</td>
            <td>${bid.dateWon}</td><td><button class="bids-view-btn" data-id="${bid.bid_id}">View</button></td>`;
        return row;
    }
            ...
            .then(data => {
                if (data.success) {
                    swappedItems = data.swaps.map(tr => {
                        ...
                        return {
                            swap_id: item.swap_id, item_id: item.item_id, item: item.item_name,
                            category: item.category, dateSwapped: item.completion_date ? new Date(item.completion_date).toLocaleDateString() : '',
                            vendor: item.vendor || '-'
                        };
                    });
                    renderItems(swappedItems, "swaps");
                    const counter = document.querySelector("#profile-swaps-btn p");
                    if (counter) counter.textContent = `${swappedItems.length} Items`;
                }
            });
    }
    ...
    function createSwappedRow(swap) {
        const row = document.createElement("tr");
        row.innerHTML = `
            <td class="item">${swap.item}</td><td>${swap.category}</td>
            <td>${swap.dateSwapped}</td><td>${swap.vendor}</td>
            <td><button class="swaps-view-btn" data-id="${swap.swap_id}">View</button></td>`;
        return row;
    }
    ...
    const contents = document.querySelectorAll("[id^='profile-'][id$='-content']");

    function showContent(id) {
        contents.forEach(c => c.style.display = (c.id === id) ? (id === "profile-user-info-content" ? "flex" : "block") : "none");
    ...
    function updateTitleForSection(sectionId) {
        ...
        if (!titleContainer || !titleHeading) return;
    ...
    // Analytics helpers for titles
    function createBidsAnalytics(bids) {
        const div = document.createElement("div"); div.classList.add("analytics");
        div.innerHTML = `<div class="total-items"><h6>${bids.length}</h6><p>Items</p></div>`;
        return div;
    }
    function createSwapsAnalytics(swaps) {
        const div = document.createElement("div"); div.classList.add("analytics");
        div.innerHTML = `<div class="total-items"><h6>${swaps.length}</h6><p>Items</p></div>`;
        return div;
    }
...
// --- Modal Global Logic ---
window.openSwapModal = function(id) {
...
// ... Remaining fetchSwapOffers, populateSwapModal, handleSwapAction logic as provided in your original file ...