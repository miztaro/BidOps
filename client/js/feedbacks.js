...
export function loadRatings() {
  fetch('/bidops/server/ratings/get_ratings.php', {
    method: 'GET',
    credentials: 'include'
  })
    .then(res => res.json())
    .then(data => {
      ...
      createFeedbackAnalytics(ratings);
    })
    .catch(err => console.error('Error fetching ratings:', err));
}

function createFeedbackAnalytics(ratings) {
  ...
  const avg =
    total === 0
      ...
      : (
        ratings.reduce(
          (sum, r) => sum + (parseInt(r.rating, 10) || 0),
          0
        ) / total
      ).toFixed(1);
  ...
  titleHeading.textContent = "My Feedbacks";
